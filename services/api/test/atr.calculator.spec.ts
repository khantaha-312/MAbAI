...
describe('calculateAtr', () => {
  ...
  it('computes a known constant-range dataset correctly (no gaps)', () => {
    // Every bar has high-low = 4, no gaps between closes -> ATR should settle at 4
  ...
  it('captures a gap correctly — true range exceeds the simple high-low spread', () => {
    // Build a long flat run, then one bar that gaps far above the previous close.
    ...
    const result = calculateAtr(bars, 14)!;
    // The gap bar's true range (28) should pull ATR meaningfully above the
    // flat-period value (4) — proves gaps are being captured, not ignored.
